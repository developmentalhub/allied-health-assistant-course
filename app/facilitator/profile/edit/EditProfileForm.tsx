...
export default function EditProfileForm({ existing }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/facilitator/complete-profile", { method: "POST", body: data });
  ...
  if (success) {
    return <div style={pageStyle}><div style={innerStyle}><div style={successStyle}>✓ Profile updated! Redirecting…</div></div></div>;
  }

  return (
    <div style={pageStyle}>
      <div style={innerStyle}>
        <a href="/facilitator-hub" style={{ fontSize: "14px", color: "#6b6880", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: "6px", marginBottom: "32px" }}>← Back to Facilitator Hub</a>
        <h1 style={{ fontFamily: "var(--font-display), Fraunces, Georgia, serif", fontSize: "32px", fontWeight: 300, color: "#1e1b2e", margin: "0 0 8px" }}>Edit your profile</h1>
        <p style={{ fontSize: "15px", color: "#6b6880", margin: "0 0 40px" }}>Changes are saved immediately and visible on the directory.</p>
