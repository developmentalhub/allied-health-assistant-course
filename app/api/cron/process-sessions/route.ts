...
export async function GET(request: NextRequest) {
  // Verify this is being called by Vercel Cron and not a random visitor
  ...
  // Find sessions starting in the next 24-25hr window (so we only process them once)
  ...
  for (const session of sessions) {
    ...
    // Fetch all pending bookings for this session
    ...
    if (meetsMinimum) {
      // ── Session goes ahead — capture all payments ──────────────────────────
      let captureErrors = 0;
      ...
      // Send reminder emails to all families
      for (const booking of bookings ?? []) {
        try {
      ...
      // ── Session cancelled — cancel/refund all payments ─────────────────────
      let cancelErrors = 0;
      ...
      for (const booking of bookings ?? []) {
        ...
        try {
          // Cancel the payment intent (automatically refunds if already captured,
          // or simply releases the hold if still uncaptured)
      ...
      // Send cancellation emails to all families
      for (const booking of bookings ?? []) {
        try {
          const { data: profile } = await supabase
            .from("profiles")
      ...
      results.push({
        ...
        action: "cancelled",
        bookings: bookings?.length ?? 0,
        refunded: (bookings?.length ?? 0) - cancelErrors,
        cancelErrors,
      });
    }
  }
